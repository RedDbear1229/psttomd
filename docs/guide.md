...
3. FTS5 `prefix='2 3 4'` 인덱스 적용 여부
4. 파일 수 vs DB `messages` 행 수 일치
5. `mailgrep` 한글 부분일치 검색
6. `mailview --doctor` (`fts prefix ✓`, `diff=+0`)
7. P5 카운트 drift 감지 — mtime 보존된 새 MD 파일을 만들고 doctor 가
   `diff=+1` 을 보고하는지

모두 ✓ 면 임시 디렉터리를 자동 정리한다. 디버깅이 필요하면
`bash scripts/wsl_smoke.sh --keep` 으로 임시 아카이브를 유지할 수 있다.
...
# 4) 환경 진단 — ✓ 만 출력되어야 정상
#    run_doctor 는 --archive 플래그 대신 MAIL_ARCHIVE 로만 root 를 받는다.
MAIL_ARCHIVE="$HOME/mv-smoke" mailview --doctor
